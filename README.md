...
- 重构agent的实现，使用deepagent来实现 -- done
- 聊天页面左侧需要有会话分组 -- done
...
- 和LLM聊天：
  - 1、支持rag，这个如何使用？-- done
  - 2、我想让他执行电脑上的工具，比如某个命令；-- done
  - 3、自定义的skill如何喂投给他
- 手机端：
  - 支持编辑笔记 -- done
  - bug：笔记/文章点击的详情，不会弹出modal -- done
  - 临时文章展示url可以加长 -- done
  - 文章展示上标题也需要加长 -- done
  - 增加图片列表和上传图片  -- done
- 桌面端：
  - 文章增加AI总结，点击就弹出框，展示总结中，总结完成给到总结内容  -- done
  - bug：文章列表，点击文章的详情不会弹出详情框  -- done
  - 聊天页面，左侧增加skill的管理 -- done
  - 内置几个工具，比如创建笔记，查询笔记，查询天气，设置闹钟等 -- done
  - 日报，周报，月报生成和管理 -- done
  - 文章和报告都在图标前面新增点击生成预览图的功能 -- done
  - 报告生成的提示词进行优化，报告图片模版升级 -- done
  - 安装和登录页面UI支持 -- done
  - 文章生成图片的模版需要优化下字体大小 -- done
- 插件端：
  - 文章总结和生成概览图，并且图片可以上传到图片库 -- done